...
Following a liquidity event and the expiration of WLTH's lock-up period, open **Portfolio** and tap **Claim**. The Slice converts to USDC, net of any applicable fees. You can keep that USDC or swap into a public ticker (for example SPCX) in the same wallet. See [From Slice to public asset](/investment/guides/from-slice-to-public-asset).
...
For the product path after unlock — Claim to USDC, then an optional swap into a public ticker — see [From Slice to public asset](/investment/guides/from-slice-to-public-asset).

For how WLTH accounts for dilution and corporate actions while a position is still locked after listing, see [Understanding Adjusted Market Cap](/investment/exclusive-access/adjusted-market-cap).
